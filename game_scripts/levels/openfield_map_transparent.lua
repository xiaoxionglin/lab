...
function api:hasEpisodeFinished(timeSeconds)
  return api._has_goal
end

function api:handlePickup(pickup)
  print("Pickup collected:", pickup.classname)
  if pickup.classname == 'goal' then
    api._has_goal = true
    game:addScore(pickup.quantity or 10)
    game:finishMap()
  end
end

function api:trigger(spawnId, targetName)
  print("Pickup collected:", spawnId, targetName)
